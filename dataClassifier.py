...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    #print 'width:%d ', DIGIT_DATUM_WIDTH
    #print 'height:%d ', DIGIT_DATUM_HEIGHT
    ...
    while (dimx <= DIGIT_DATUM_WIDTH/2) or (dimy <= DIGIT_DATUM_HEIGHT/2 ):
        if True:

            ulx = 0
            uly = 0

            left = DIGIT_DATUM_WIDTH
            right = 0
            ...
            while ulx < DIGIT_DATUM_WIDTH: 
                while uly < DIGIT_DATUM_HEIGHT:

                    x = ulx
                    y = uly
                    count_on = 0
                    count_off = 0
                    while x < min((ulx+dimx), DIGIT_DATUM_WIDTH):
                        while y < min((uly+dimy), DIGIT_DATUM_HEIGHT):
                            if datum.getPixel(x, y) > 0:
                                count_on += 1
                            else:
                                count_off += 1
                    ...
                    if count_on > count_off + 1:
                        features['(%d,%d)(%d,%d)', dimx, dimy, ulx, uly] = 1
                        ...
                        features['(%d,%d)(%d,%d)', dimx, dimy, ulx, uly] = 0

                    # if count_on >= 2*count_off:
                    #     features['strong(%d,%d)(%d,%d)', dimx, dimy, ulx, uly] = 1
                    # else:
                    #     features['strong(%d,%d)(%d,%d)', dimx, dimy, ulx, uly] = 0

                    # if dim == 2:
                    #     if (right - left < my_width) and (right - left > 0):
                    #         narrow_count += 1
                    #         features['narrow'] = 1
                    #     else:
                    #         fat_count += 1
...
def enhancedPacmanFeatures(state, action):
    ...
    foodCount2 = s2.getFood().count()

    pac2 = s2.getPacmanPosition()

    food_distance = 1000000
    food = state.getFood().asList()
    for p in food:
        cand = abs(p[0]-pac2[0]) + abs(p[1]-pac2[1]) 
        if cand < food_distance: #maybe keep this distance
            food_distance = cand
    # if food_distance == 1234567:
    #     food_distance = 0

    distance2 = 1000000
    for p in s2.getGhostPositions():
        cand = abs(p[0]-pac2[0]) + abs(p[1]-pac2[1]) 
        if cand < distance2: #maybe keep this distance
            distance2 = cand
    if distance2 == 1000000:
        distance2 = 0

    features['closest ghost'] = distance2/4 
    features['closest food'] = food_distance 
    return features
